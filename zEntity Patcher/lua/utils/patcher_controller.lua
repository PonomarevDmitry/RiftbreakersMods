...
function controller:EnsurePatch()
    ...
    local function PatchComponents( blueprint, component, Handler )
        local bp = ResourceManager:GetBlueprint( blueprint )
        if not bp then
            return nil
        end
        ...
        if not et_component then
            return false
        end

        Handler( divergent_helper( et_component ), divergent_helper( bp:GetComponent( component ) ) )
    ...
    local function EnsureWeaponItemDesc()
        ...
        local function PatchEntityWeaponComponent( entity, bp )
            local et_component = EntityService:GetConstComponent( entity, component )
            if et_component ~= nil then
                et_component = divergent_helper( et_component )
                local bp_component = divergent_helper( bp:GetComponent( component ) )
        ...
        for blueprint, _ in pairs( patch_content.weapons ) do
            local bp = ResourceManager:GetBlueprint( blueprint )
            if bp then
                local entity = INVALID_ID

                if blueprint:sub( -5 ) == "_item" then
                    entity = ItemService:SpawnLoot( position, blueprint, 0, 0, 0, "" )
                else
                    entity = EntityService:SpawnEntity( blueprint, position, 1 )
                end

                PatchEntityWeaponComponent( entity, bp )
                EntityService:RemoveEntity( entity )
            end
        end
    end
    ...
    local function EnsureTurretDescTarget()
        ...
        local function Handler( et_component, bp_component )
            local et_target = et_component.target
            local bp_target = bp_component.target
        ...
        for blueprint, _ in pairs( patch_content.turret ) do
            if PatchComponents( blueprint, component, Handler ) == false then
                LogService:Log( ("Fail to patch '%s' of %s"):format( component, blueprint ) )
            end
        end
    end
