...
class 'power_wells_activate_tool' ( tool )

function power_wells_activate_tool:__init()
    tool.__init(self,self)
end

function power_wells_activate_tool:OnInit()
    self.childEntity = EntityService:SpawnAndAttachEntity("misc/marker_selector_power_wells_activate_tool", self.entity)
end

function power_wells_activate_tool:OnPreInit()
    self.initialScale = { x=8, y=1, z=8 }
end

function power_wells_activate_tool:SpawnCornerBlueprint()
    if ( self.corners == nil ) then
        self.corners = EntityService:SpawnAndAttachEntity("misc/marker_selector_corner_tool", self.entity )
    end
end

function power_wells_activate_tool:FindEntitiesToSelect( selectorComponent )
...
function power_wells_activate_tool:AddedToSelection( entity )
...
function power_wells_activate_tool:RemovedFromSelection( entity )
...
function power_wells_activate_tool:OnRotate()
end

function power_wells_activate_tool:OnActivateSelectorRequest()
...
function power_wells_activate_tool:OnActivateEntity( entity )

    if ( not EntityService:IsAlive( entity ) ) then
        return
    end

    local mapperName = "PowerWellActivateRequest|" .. tostring(self.playerId)

    QueueEvent("OperateActionMapperRequest", entity, mapperName, false )
end

return power_wells_activate_tool
