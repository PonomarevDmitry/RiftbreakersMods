...
class 'power_wells_preserve_tool' ( tool )

function power_wells_preserve_tool:__init()
    tool.__init(self,self)
end

function power_wells_preserve_tool:OnInit()
    self.childEntity = EntityService:SpawnAndAttachEntity("misc/marker_selector_power_wells_preserve_tool", self.entity)
end

function power_wells_preserve_tool:OnPreInit()
    self.initialScale = { x=8, y=1, z=8 }
end

function power_wells_preserve_tool:SpawnCornerBlueprint()
    if ( self.corners == nil ) then
        self.corners = EntityService:SpawnAndAttachEntity("misc/marker_selector_corner_tool", self.entity )
    end
end

function power_wells_preserve_tool:FindEntitiesToSelect( selectorComponent )
...
function power_wells_preserve_tool:AddedToSelection( entity )
...
function power_wells_preserve_tool:RemovedFromSelection( entity )
...
function power_wells_preserve_tool:OnRotate()
end

function power_wells_preserve_tool:OnActivateSelectorRequest()
...
function power_wells_preserve_tool:OnActivateEntity( entity )
...
return power_wells_preserve_tool
