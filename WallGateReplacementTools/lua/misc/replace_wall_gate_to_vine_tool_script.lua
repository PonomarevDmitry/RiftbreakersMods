...
class 'replace_wall_gate_to_vine_tool_script' ( LuaEntityObject )

function replace_wall_gate_to_vine_tool_script:__init()
    LuaEntityObject.__init(self, self)
end

function replace_wall_gate_to_vine_tool_script:init()
...
function replace_wall_gate_to_vine_tool_script:OnLoad()
...
function replace_wall_gate_to_vine_tool_script:OnBuildingSellEndEvent()
    ...
    for deltaX in Iter( deltas ) do
        for deltaZ in Iter( deltas ) do
            ...
            newTransform.orientation = CreateQuaternion( { x=0, y=1, z=0 }, randomAngle )
...
function replace_wall_gate_to_vine_tool_script:AddToHash(hashPositions, newPositionX, newPositionZ)
...
function replace_wall_gate_to_vine_tool_script:DestroySelf()
    self.targetEntity = nil
    EntityService:RemoveEntity( self.entity )
end

return replace_wall_gate_to_vine_tool_script