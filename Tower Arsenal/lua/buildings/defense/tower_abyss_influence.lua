...
class 'tower_albyss_influence' ( tower )

function tower_albyss_influence:__init()
	tower.__init(self,self)
end

function tower_albyss_influence:OnInit()
    tower.OnInit(self)
...
function tower_albyss_influence:OnSpikeAmmoFiredEvent( evt )
	AnimationService:StartAnim( self.entity, "working", false )
end

function tower_albyss_influence:OnExecuteChecker( state )
...
function tower_albyss_influence:OnWorkInProgress( state )
...
function tower_albyss_influence:OnWorkExit( state )
	for i = #self.selected,1,-1  do		
		ItemService:RemoveHealthLink( self.selected[i], self.healthChild )
	end		
	Clear( self.selected )
...
function tower_albyss_influence:OnActivate()
	self.fsm:ChangeState("working")
end

function tower_albyss_influence:OnDeactivate()
