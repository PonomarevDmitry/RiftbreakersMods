...
class 'ta_reflection_shield_generator' (building)
...
function ta_reflection_shield_generator:__init()
	building.__init(self, self)
end

function ta_reflection_shield_generator:OnInit()
...
function ta_reflection_shield_generator:OnBuildingEnd()
	ItemService:AddHealthLink(self.entity, self.shield_ent)
end

function ta_reflection_shield_generator:OnActivate()
	self.sm_1:ChangeState("running")
	self:ChargeShield()
end

function ta_reflection_shield_generator:OnDeactivate()
	self:DrainShield()
	self.sm_1:ChangeState("draining")
end

function ta_reflection_shield_generator:OnSell()
...
function ta_reflection_shield_generator:OnDestroy()
...
function ta_reflection_shield_generator:Start_Running(state)
	self.sm_2:ChangeState("searching")
end

function ta_reflection_shield_generator:Loop_Running(state)
...
function ta_reflection_shield_generator:End_Running(state)
...
function ta_reflection_shield_generator:Loop_Searching(state)
...
function ta_reflection_shield_generator:Loop_Draining(state)
...
function ta_reflection_shield_generator:Start_Blown(state)
...
function ta_reflection_shield_generator:Loop_Blown(state)
...
function ta_reflection_shield_generator:DeregisterAll()
...
function ta_reflection_shield_generator:RebuildReflection()
	for obj, status in pairs(self.selected) do
		if status == 0 then
			self.selected[obj] = self:SetReflection(obj)
		end
...
function ta_reflection_shield_generator:FlagAllForRebuild()
...
function ta_reflection_shield_generator:SetReflection(target)
...
function ta_reflection_shield_generator:RemoveReflection(target)
...
function ta_reflection_shield_generator:ChargeShield()
	local R_component = reflection_helper(EntityService:GetComponent(self.shield_ent, "RegenerationComponent"))
	R_component.regeneration = self.regen_rate
	R_component.regeneration_cooldown = self.regen_delay
end

function ta_reflection_shield_generator:DrainShield()
	local R_component = reflection_helper(EntityService:GetComponent(self.shield_ent, "RegenerationComponent"))
...
return ta_reflection_shield_generator