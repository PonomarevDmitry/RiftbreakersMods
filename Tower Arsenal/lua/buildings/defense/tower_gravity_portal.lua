...
class 'tower_gravity_portal' ( tower )

function tower_gravity_portal:__init()
	tower.__init(self,self)
end

function tower_gravity_portal:OnInit()
    tower.OnInit(self)
...
function tower_gravity_portal:OnActivate()
...
function tower_gravity_portal:OnDeactivate()
...
function tower_gravity_portal:OnWorkingEnter( state )
...
function tower_gravity_portal:OnWorkingExecute( state )
...
function tower_gravity_portal:OnWorkingExit( state )
	if self.readyBomb ~= nil then
...
function tower_gravity_portal:OnShootingEnter( state )
	EffectService:DestroyEffectsByGroup( self.entity, "portal_pending" )
  	EntityService:SetGraphicsUniform( self.entity, "cGlowFactor", 2 )
end

function tower_gravity_portal:OnShootingExecute( state )
	local progress = WeaponService:GetWeaponReloadProgress( self.entity )
	if progress > 0.9 then
		if self.portalTriangle == nil then 
			self.portalTriangle = EntityService:SpawnAndAttachEntity( "buildings/defense/tower_gravity_portal_triangle", self.entity, "att_triangle", "" )
...
function tower_gravity_portal:OnShootingStartEnter( state )
...
function tower_gravity_portal:OnShootingStartExecute( state )
...
function tower_gravity_portal:OnShootingStartExit( state )
	EffectService:DestroyEffectsByGroup( self.entity, "portal_ready" )
end

function tower_gravity_portal:OnShootingDelayEnter( state )
	state:SetDurationLimit( 0.16 )
end

function tower_gravity_portal:OnShootingDelayExit( state )
   self.sfsm:ChangeState("shooting_end")
end

function tower_gravity_portal:OnShootingEndEnter( state )
	AnimationService:StartAnim( self.portalTriangle, "hide_portal", false, 3 )
	EffectService:AttachEffects( self.entity, "portal_pending" )
end

function tower_gravity_portal:OnShootingEndExecute( state )
...
function tower_gravity_portal:OnShootingEndExit( state )
	EffectService:DestroyEffectsByGroup( self.entity, "portal_pending" )
  	EntityService:SetGraphicsUniform( self.entity, "cGlowFactor", 2 )
end

function tower_gravity_portal:OnShootingDisabledEnter( state )
	if self.portalTriangle ~= nil then
