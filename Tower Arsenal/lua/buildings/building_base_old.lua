...
class 'building_base_old' ( day_cycle_machine )

function building_base_old:__init()
...
function building_base_old:init()
	self.version = 2
...
function building_base_old:CreateBuildingStateMachine()
...
function building_base_old:GetMaterials()
...
function building_base_old:OnRelease()
	self:OnRemove()
	self:RemoveAllBuilidingEntities()
end

function building_base_old:OnDestroyRequest(evt)
	self.meshEnt = BuildingService:GetMeshEntity(self.entity);
	self:OnDestroy(evt)
end

function building_base_old:OnDestroy()
end

function building_base_old:CreateBaseCubes()
...
function building_base_old:_OnBuildNew(evt)
	self:OnCubeFlyStart()
end

function building_base_old:OnStartBuildingEvent()
	self:OnBuildingStart()
end

function building_base_old:_OnBuild(evt)
	return
end

function building_base_old:_OnSell(evt)
	self.meshEnt = BuildingService:GetMeshEntity(self.entity);
...
function building_base_old:_OnOverride(evt)
	self:OnOverride()
end

function building_base_old:_OnBuildingModifiedEvent(evt)
end

function building_base_old:_OnWaitEnter( state )
	state:SetDurationLimit( self.nextStateWaitDuration )
end

function building_base_old:_OnWaitExit( state )
	if (self.buildingSell == true  ) then
		return
	end
...
function building_base_old:_OnCubeFlyEnter( state )
...
function building_base_old:_OnCubeFlyExecute( state )
	if(self.forwardTime > 0 ) then
		state:Exit();
	end	
end

function building_base_old:_OnCubeFlyExit( state )
...
function building_base_old:OnMoveEndEvent( evt )
	if (self.buildingSM ~= nil and  evt:GetEntity() == self.cubeEnt) then
		self.buildingSM:Deactivate()
	end
end

function building_base_old:_OnCubeFlySellEnter( state )
	if ( AnimationService:HasAnim( self.cubeEnt, "fly_and_scale") ) then
		AnimationService:StartAnim( self.cubeEnt, "fly_and_scale", false, 4 )
	end
	self:OnCubeFlyStart()
...
function building_base_old:_OnCubeFlySellExit( state )
...
function building_base_old:_OnWaitForSpace( state )
...
function building_base_old:_OnBuildingEnter( state )
	self:OnBuildingStart()
...
function building_base_old:_OnBuildingExecute( state )
...
function building_base_old:_OnBuildingFastForwardEvent( evt )
	self.forwardTime = self.forwardTime + evt:GetDeltaTime()
end

function building_base_old:_OnBuildingExit( state )
	self.forwardTime = 0
...
function building_base_old:_OnSellEnter( state )
	self.meshEnt = BuildingService:GetMeshEntity(self.entity);
...
function building_base_old:_OnSellExecute( state )
end

function building_base_old:_OnSellExit( state )
...
function building_base_old:_CreateLine( ent1, ent2, dir, lengthMultiplier )
...
function building_base_old:_OnBigBuildingEnterState1( state )
...
function building_base_old:_UpdateLines( progress, lines )
...
function building_base_old:_OnBigBuildingExecuteState1( state )
...
function building_base_old:_OnBigBuildingExitState1( state )
...
function building_base_old:_OnBigBuildingEnterState2( state )
    state:SetDurationLimit( self.extendLength )
	local i = 0
...
function building_base_old:_OnBigBuildingExecuteState2( state )
	local progress = state:GetDuration() / state:GetDurationLimit()
	self:_UpdateLines(progress, self.currentLines)	
	if(self.forwardTime > 0 ) then
		state:Exit();
...
function building_base_old:_OnBigBuildingExitState2( state )
...
function building_base_old:_OnBigBuildingEnterState3( state )
    state:SetDurationLimit( self.extendLength )
	local i = 0
...
function building_base_old:_OnBigBuildingExecuteState3( state )
	local progress = state:GetDuration() / state:GetDurationLimit()
	self:_UpdateLines(progress, self.currentLines)	
	if(self.forwardTime > 0 ) then
		state:Exit();
...
function building_base_old:_OnBigBuildingExitState3( state )
...
function building_base_old:_OnBigBuildingEnterState4( state )
...
function building_base_old:_OnBigBuildingExecuteState4( state )
	local progress = state:GetDuration() / state:GetDurationLimit()
	self:_UpdateLines(progress, self.currentLines)	
	if(self.forwardTime > 0 ) then
		state:Exit();
...
function building_base_old:_OnBigBuildingExitState4( state )
...
function building_base_old:_OnHideScafoldingEnter( state )
...
function building_base_old:_OnHideScafoldingExecute( state )
	local minTime = 0
...
function building_base_old:_OnHideScafoldingExit( state )
...
function building_base_old:_OnActivate()
...
function building_base_old:_OnDeactivate()
...
function building_base_old:OnActivate()
end

function building_base_old:OnDeactivate()
end

function building_base_old:OnInit()
end

function building_base_old:OnBuildingStart()
end

function building_base_old:OnBuild()
end

function building_base_old:OnCubeFlyStart()
end

function building_base_old:OnBuildingEnd()
end

function building_base_old:_OnBuildingEnd()
	self:OnBuildingEnd()
...
function building_base_old:OnSellStart()
end

function building_base_old:OnOverride()
end

function building_base_old:OnSell()
end

function building_base_old:_OnBuildingPoweredEvent()
...
function building_base_old:_OnBuildingPowerOffEvent()
...
function building_base_old:_OnBuildingResourceGranted()
...
function building_base_old:_OnBuildingResourceMissing()
...
function building_base_old:RemoveAllBuilidingEntities()
...
function building_base_old:OnRemove()
	return false
end

function building_base_old:BuildingModified()
end

function building_base_old:RemoveBuildingStateMachine()
...
function building_base_old:OnLoad()
...
function building_base_old:SetTurretMode( mode )
...
function building_base_old:InitializeDisplayRadiusHandlers()
...
function building_base_old:_OnShowHideDisplayRadius(evt)
...
function building_base_old:_OnOperateActionMenuEvent(evt)
	if not self.display_radius_size then
		return
	end
...
function building_base_old:UpdateDisplayRadiusVisibility( show, entity )
	if show then
...
-- INVISIBILITY CODE!
-- =================================================================================================
function building_base_old:OnEnterInvisiblityEvent(evt)
	self.invisibility = true;
	self.invisibilityFsm:ChangeState( "invisibility" )
end

function building_base_old:OnExitInvisiblityEvent(evt)
	self.invisibility = false;
...
function building_base_old:OnInvisibilityEnter( state )
	...
	if ( EntityService:IsSkinned( self.entity )) then
		...
		EntityService:SetMaterial( self.entity, "buildings/invisibility", "invisiblity" )
	end
	...
	for child in Iter(children) do
			...
			for meshChild in Iter(meshChildren) do
				QueueEvent( "FadeEntityOutRequest", meshChild, 1.5 ) -- + RandFloat( -0.33, 0.33 ) )
				if ( EntityService:IsSkinned( meshChild )) then
					EntityService:SetMaterial( meshChild, "buildings/invisibility_skinned", "invisiblity" )
				else
					EntityService:SetMaterial( meshChild, "buildings/invisibility", "invisiblity" )
				end
			end
	end
...
function building_base_old:OnInvisibilityExit( state )
