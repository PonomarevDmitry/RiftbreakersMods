...
class 'cosmic_mech_bot' ( mech )

function cosmic_mech_bot:__init()
	mech.__init( self,self )
end

function cosmic_mech_bot:init()
	self.is_bot = true;
	mech.init(self)
end
...
function cosmic_mech_bot:_OnInit()
...
function cosmic_mech_bot:PrepareBotEquipment()
	self.useWeapon = false
...
function cosmic_mech_bot:OnBotItemEquippedEvent(evt)
...
function cosmic_mech_bot:OnBotItemUnequippedEvent(evt)
	if evt:GetSlot() ~= self.LEFT_HAND and evt:GetSlot() ~= self.RIGHT_HAND then
		return
	end
...
function cosmic_mech_bot:PickRandomWeapon( type )
...
function cosmic_mech_bot:OnEquipmentSlotChangedEvent( event )
	self.useWeapon = self.currentWeaponInUse.item ~= nil
end

function cosmic_mech_bot:OnStartMeleeEvent( evt )
	self:PickRandomWeapon( self.PICK_MELEE_WEAPON )
end

function cosmic_mech_bot:OnStopMeleeEvent( evt )
	self.useWeapon = false
...
function cosmic_mech_bot:OnShootEvent( evt )
	self:PickRandomWeapon( self.PICK_RANGE_WEAPON )
end
-- waponstatcomponent WeaponItemComponent
function cosmic_mech_bot:OnStopShootEvent( evt )
	self.useWeapon = false
...
function cosmic_mech_bot:OnExecuteAttack( state, dt )
...
function cosmic_mech_bot:EnableMechFunctionality( value )
...
function cosmic_mech_bot:IsEquipped( item )
...
function cosmic_mech_bot:AddItem( item )
...
function cosmic_mech_bot:RemoveItem( item )
	if item == INVALID_ID then
		return
	end
...
function cosmic_mech_bot:OnLuaGlobalEvent( evt )
