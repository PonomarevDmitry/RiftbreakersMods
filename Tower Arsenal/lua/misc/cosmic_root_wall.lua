class 'cosmic_root_wall' ( LuaEntityObject )

function cosmic_root_wall:__init()
	LuaEntityObject.__init(self,self)
end

function cosmic_root_wall:init()
...
function cosmic_root_wall:OnWaitEnter( state )
...
function cosmic_root_wall:OnWaitExit( state )
	self.sm:ChangeState( "move" )
end

function cosmic_root_wall:OnMoveEnter( state )
	self.position = EntityService:GetPosition( self.entity )
end

function cosmic_root_wall:OnMoveExecute( state, dt )
	...
	if ( self.onTrigger == false ) and ( self.position.y > ( self.originToStopAboveGroundHeight - 3.0 ) ) then 
		...
		for i =1,rocksCount do
			local ent = ItemService:SpawnLoot( self.entity, "units/ground/cosmic_gulgor_boss/crystal", 30.0, 6000, 12000, "normal" )
			EffectService:AttachEffect( ent, "effects/destructibles/crystal_blue_big_part_trail" )
...
function cosmic_root_wall:OnRemoveEnter( state )
	state:SetDurationLimit( self.lifeTime )
end

function cosmic_root_wall:OnRemoveExit( state )
	EntityService:DissolveEntity( self.entity, 1.0 )
end


return cosmic_root_wall
