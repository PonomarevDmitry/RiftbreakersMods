...
class 'mission_the_abys_outpost' ( mission_base )

function mission_the_abys_outpost:__init()
    mission_base.__init(self,self)
end

function mission_the_abys_outpost:EnsureInfluencePylonsObjective()
...
function mission_the_abys_outpost:UpdateMissionProgress()
    local mission_progress = 
    {
        { mission_name = "the_abys_crash_site", data_source = "missions/campaigns/the_abyss/the_abys_outpost_stage_2.mission" },
        { mission_name = "the_abys_factory",    data_source = "missions/campaigns/the_abyss/the_abys_outpost_stage_3.mission" }
    }
...
function mission_the_abys_outpost:OnLoad()
...
function mission_the_abys_outpost:init()
    mission_base.init(self)

    self:PrepareSpawnPoints();

    MissionService:ActivateMissionFlow("", "logic/missions/campaigns/the_abyss/the_abys_outpost.logic", "default" )

    local rulesPath = GetRulesForDifficulty( "lua/missions/campaigns/the_abyss/dom_the_abys_outpost_rules_" )
    MissionService:AddGameRule( "lua/missions/v2/dom_manager.lua", rulesPath )
end

return mission_the_abys_outpost