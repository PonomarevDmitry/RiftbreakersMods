...
class 'replace_wall_picker_tool' ( replace_wall_base )
...
function replace_wall_picker_tool:SetBuildingIcon()
    ...
    if ( self.selectedMode >= self.modeBuildingLastSelected ) then
        ...
        if ( menuIcon ~= "" ) then
            ...
            messageText = "${gui/hud/messages/replace_wall_tool/last_building} " .. tostring(indexBuilding + 1) .. ": ${" .. buildingDescRef.localization_id .. "}"
        end
    ...
    elseif ( self.selectedBuildingBlueprint ~= "" and ResourceManager:ResourceExists( "EntityBlueprint", self.selectedBuildingBlueprint ) ) then
        ...
        if ( menuIcon ~= "" ) then
            ...
            messageText = "${gui/hud/messages/replace_wall_tool/current_building} ${" .. buildingDescRef.localization_id .. "}"
        end
    end
    ...
    markerDB:SetString("wall_name", messageText)
end
...
function replace_wall_picker_tool:ChangeSelector(blueprintName)
...
function replace_wall_picker_tool:FillLastBuildingsList(defaultModesArray, modeBuildingLastSelected, selector)
...
function replace_wall_picker_tool:AddBlueprintToLastList(blueprintName, selector)

    LastSelectedBlueprintsListUtils:AddBlueprintToList(self.list_name, selector, blueprintName)
end

function replace_wall_picker_tool:OnRotateSelectorRequest(evt)
...
function replace_wall_picker_tool:CheckModeValueExists( selectedMode )
