...
function trap:OnTimerElapsedEvent( evt )
    ...
    if (self.numberOfActivations <= 0) then
        ...
        if BuildingService:PayRepairCosts( self.entity, player_id, -1 ) then
            local bp_data = EntityService:GetBlueprintDatabase( self.entity )
            local max_activations = bp_data:GetInt( "number_of_activations" )
            self.numberOfActivations = max_activations
            self.data:SetInt( "number_of_activations", max_activations )
        else
    ...
    if (self.entitiesInsideCount > 0) then
