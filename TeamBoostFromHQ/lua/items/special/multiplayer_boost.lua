...
function multiplayer_boost:OnChargingInProgress( state )
    ...
    if ( currentTime >self.cooldownDeadline ) then
        ...
        -- TeamBoostFromHQ Change Start

        local objects = FindService:FindEntitiesByTypeInRadius( self.owner, "headquarters", self.dischargeRadius )

        ConcatUnique( mechs, objects )

        -- TeamBoostFromHQ Change Finish
    end
