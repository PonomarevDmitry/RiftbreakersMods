...
class 'replace_trap_picker_tool' ( replace_trap_base )

function replace_trap_picker_tool:__init()
    replace_trap_base.__init(self,self)
end

function replace_trap_picker_tool:OnPreInit()
    self.initialScale = { x=1, y=1, z=1 }
end

function replace_trap_picker_tool:GetScaleFromDatabase()
    return { x=1, y=1, z=1 }
end

function replace_trap_picker_tool:OnInit()
...
function replace_trap_picker_tool:SetBuildingIcon()
    ...
    if ( self.selectedMode >= self.modeBuildingLastSelected ) then
        ...
        if ( menuIcon ~= "" ) then
            ...
            messageText = "${gui/hud/messages/replace_trap_tool/last_building} " .. tostring(indexBuilding + 1) .. ": ${" .. buildingDescRef.localization_id .. "}"
        end
    ...
    markerDB:SetInt("trap_icon_visible", buildingIconVisible)
    markerDB:SetString("trap_icon", buildingIcon)
    markerDB:SetString("trap_name", messageText)
end

function replace_trap_picker_tool:SpawnCornerBlueprint()
    if ( self.corners == nil ) then
        self.corners = EntityService:SpawnAndAttachEntity( "misc/marker_selector_corner_tool", self.entity )
    end
end

function replace_trap_picker_tool:AddedToSelection( entity )
...
function replace_trap_picker_tool:RemovedFromSelection( entity )
    EntityService:RemoveMaterial(entity, "selected" )
end

function replace_trap_picker_tool:FilterSelectedEntities( selectedEntities )
...
function replace_trap_picker_tool:OnActivateSelectorRequest()
...
function replace_trap_picker_tool:ChangeSelector(blueprintName)
...
function replace_trap_picker_tool:AddBlueprintToLastList(blueprintName, selector)

    LastSelectedBlueprintsListUtils:AddBlueprintToList(self.list_name, selector, blueprintName)
end

function replace_trap_picker_tool:OnRotateSelectorRequest(evt)
...
function replace_trap_picker_tool:CheckModeValueExists( selectedMode )
...
return replace_trap_picker_tool