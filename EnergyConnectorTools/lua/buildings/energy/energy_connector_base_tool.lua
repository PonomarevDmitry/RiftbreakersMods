...
class 'energy_connector_base_tool' ( LuaEntityObject )

function energy_connector_base_tool:__init()
    LuaEntityObject.__init(self,self)
end

function energy_connector_base_tool:init()
...
function energy_connector_base_tool:InitializeValues()
    ...
    self.connectorBlueprintName = "buildings/energy/energy_connector"

    self:FillConnectorBuildingDesc( self.connectorBlueprintName )

    self:FindMinDistance()
end

function energy_connector_base_tool:CreateInfoChild()
...
function energy_connector_base_tool:FindMinDistance()

    self.radius = BuildingService:FindEnergyRadius( self.connectorBlueprintName )
...
function energy_connector_base_tool:FillConnectorBuildingDesc(connectorBlueprintName)

    local buildingDesc = reflection_helper( BuildingService:GetBuildingDesc( connectorBlueprintName ) )

    self.ghostBlueprintName = buildingDesc.ghost_bp
    self.buildingDesc = buildingDesc
end

function energy_connector_base_tool:SpawnGhostConnectorEntity(position, orientation)
...
function energy_connector_base_tool:OnWorkExecute()
...
function energy_connector_base_tool:CheckEntityBuildable( entity, transform, id )

    id = id or 1
    local test = nil

    test = BuildingService:CheckGhostBuildingStatus( self.playerId, entity, transform, self.connectorBlueprintName, id )
...
function energy_connector_base_tool:ChangeEntityMaterial( entity, material )
...
function energy_connector_base_tool:BuildEntity(entity, transform, createCube)
...
function energy_connector_base_tool:BuildDesertFloor(spot)
...
function energy_connector_base_tool:ShouldBuildDesertFloor( position )
...
function energy_connector_base_tool:OnActivateSelectorRequest()
end

function energy_connector_base_tool:OnDeactivateSelectorRequest()
end

function energy_connector_base_tool:OnRotateSelectorRequest()
end

function energy_connector_base_tool:OnRelease()
...
return energy_connector_base_tool