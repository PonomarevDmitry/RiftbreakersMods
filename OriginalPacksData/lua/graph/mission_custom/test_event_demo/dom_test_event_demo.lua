class 'dom_test_event_demo' ( LuaGraphNode )

function dom_test_event_demo:__init()
	LuaGraphNode.__init(self,self)
end

function dom_test_event_demo:init()    
...
function dom_test_event_demo:Activated()		
	self.fsm:ChangeState("wait")
end

function dom_test_event_demo:RandomizeSpawnPoint()
...
function dom_test_event_demo:OnEnterWait( state )
	...
	if ( #FindService:FindEntitiesByBlueprint( "buildings/main/headquarters" ) > 0 ) and ( #FindService:FindEntitiesByBlueprint( "buildings/defense/basic_tower" ) > 9 ) then		
		MissionService:ActivateMissionFlow( "", "logic/missions/test/event_demo/wave_level_3.logic", "default", self.data )
	...
	elseif ( #FindService:FindEntitiesByBlueprint( "buildings/main/headquarters" ) > 0 ) and ( #FindService:FindEntitiesByBlueprint( "buildings/defense/basic_tower" ) > 5 ) then
		MissionService:ActivateMissionFlow( "", "logic/missions/test/event_demo/wave_level_2.logic", "default", self.data )
		--state:SetDurationLimit( 30 )
	elseif ( #FindService:FindEntitiesByBlueprint( "buildings/main/headquarters" ) > 0 ) then		
		MissionService:ActivateMissionFlow( "", "logic/missions/test/event_demo/wave_level_1.logic", "default", self.data )
	end	
	
	
end

function dom_test_event_demo:OnExitWait( state )
