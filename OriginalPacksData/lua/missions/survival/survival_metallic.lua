...
class 'survival_metallic' ( survival_base )

function survival_metallic:__init()
    survival_base.__init(self,self)
end

function survival_metallic:init()
	survival_base.init(self)

	LogService:Log( "survival_metallic:init()" )
    ...
    local rulesPath = GetRulesForDifficulty( "lua/missions/survival/v2/dom_survival_metallic_rules_" )

	MissionService:AddGameRule( "lua/missions/v2/dom_manager.lua", rulesPath )
end

return survival_metallic
