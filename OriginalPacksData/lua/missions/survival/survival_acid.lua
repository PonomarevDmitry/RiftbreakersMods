...
class 'survival_acid' ( survival_base )

function survival_acid:__init()
    survival_base.__init(self,self)
end

function survival_acid:init()
	survival_base.init( self )

	LogService:Log( "survival_acid:init()" )
    ...
    local rulesPath = GetRulesForDifficulty( "lua/missions/survival/v2/dom_survival_acid_rules_" )

	MissionService:AddGameRule( "lua/missions/v2/dom_manager.lua", rulesPath )
end

return survival_acid
