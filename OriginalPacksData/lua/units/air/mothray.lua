...
class 'mothray' ( LuaEntityObject )

function mothray:__init()
	LuaEntityObject.__init(self, self)
end

function mothray:init()
...
function mothray:OnDestroyRequest( evt )
...
function mothray:_OnIdleEnter( state )
	self.stateTimer = -math.random(0.0,10.0)
end
function mothray:_OnIdleExecute( state, dt )
