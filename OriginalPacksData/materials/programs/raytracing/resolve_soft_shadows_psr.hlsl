...
RegularBuffer<uint>                   SobolBuffer           : register(t3);
RegularBuffer<uint>                   ScramblingTileBuffer  : register(t4);
...
#elif Z_LIGHT_TYPE == LIGHT_TYPE_POINT
    const float3 light_direction = LightPosition.xyz - world_position;
    const float distance_to_light = length(light_direction);
    is_valid = is_valid && !(distance_to_light > LightRadius);
#elif Z_LIGHT_TYPE == LIGHT_TYPE_SPOTLIGHT
    const float3 light_direction = LightPosition.xyz - world_position;
    const float distance_to_light = length(light_direction);
    const float angle_to_light = acos( dot( normalize( light_direction ), -LightDirection.xyz ) );
...
#if Z_LIGHT_TYPE == LIGHT_TYPE_DIRECTIONAL
    const float3 ray_direction = normalize(MapToCone(s, -LightDirection.xyz, LightDirection.xyz, LightSize));
#elif Z_LIGHT_TYPE == LIGHT_TYPE_POINT
    const float3 ray_direction = normalize(MapToCone(s, LightPosition.xyz, normalize(light_direction), LightSize) - world_position);
#elif Z_LIGHT_TYPE == LIGHT_TYPE_SPOTLIGHT
    const float3 ray_direction = normalize(MapToCone(s, LightPosition.xyz, normalize(light_direction), LightSize) - world_position);
#endif
