...
class 'pipe_area_tool' ( pipe_base_tool )

function pipe_area_tool:__init()
    pipe_base_tool.__init(self,self)
end

function pipe_area_tool:OnInit()
...
function pipe_area_tool:OnUpdate()
    ...
    if ( self.nowBuildingLine and self.buildStartPosition ) then
        ...
        local currentTransform = EntityService:GetWorldTransform( self.entity )
        local testBuildable = self:CheckEntityBuildable( self.ghostPipe, currentTransform )

        if ( testBuildable ~= nil) then
            self:AddToEntitiesToSellList(testBuildable)
        end

        --BuildingService:CheckAndFixBuildingConnection(self.ghostPipe)
    end
...
function pipe_area_tool:CreateNewEntity(newPosition, orientation, team)
    ...
    if ( self.ghostBlueprintName ~= "" and self.ghostBlueprintName ~= nil ) then
        ...
        result = EntityService:SpawnEntity( self.pipeBlueprintName, newPosition, team )
    end
...
function pipe_area_tool:FindPositionsToBuildLine(buildStartPosition, buildEndPosition)

    local gridSize = BuildingService:GetBuildingGridSize(self.ghostPipe)
...
function pipe_area_tool:OnActivateSelectorRequest()
...
function pipe_area_tool:OnDeactivateSelectorRequest()
    self:FinishLineBuild()

    self:RemoveMaterialFromOldBuildingsToSell()
end

function pipe_area_tool:FinishLineBuild()
...
function pipe_area_tool:GetAllEntities()
...
function pipe_area_tool:OnRelease()
