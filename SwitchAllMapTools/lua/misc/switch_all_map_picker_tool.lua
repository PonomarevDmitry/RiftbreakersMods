...
class 'switch_all_map_picker_tool' ( switch_all_map_base )

function switch_all_map_picker_tool:__init()
    switch_all_map_base.__init(self,self)
end

function switch_all_map_picker_tool:OnPreInit()
    self.initialScale = { x=1, y=1, z=1 }
end

function switch_all_map_picker_tool:GetScaleFromDatabase()
    return { x=1, y=1, z=1 }
end

function switch_all_map_picker_tool:OnInit()
...
function switch_all_map_picker_tool:SetBuildingIcon()
    ...
    elseif ( self.selectedBuildingBlueprint ~= "" and ResourceManager:ResourceExists( "EntityBlueprint", self.selectedBuildingBlueprint ) ) then
        ...
        if ( menuIcon ~= "" ) then
            ...
            messageText = "${gui/hud/switch_all_map/current_building} ${" .. buildingDescRef.localization_id .. "}"
        end
    end
...
function switch_all_map_picker_tool:SpawnCornerBlueprint()
    if ( self.corners == nil ) then
        self.corners = EntityService:SpawnAndAttachEntity( "misc/marker_selector_corner_tool", self.entity )
    end
end

function switch_all_map_picker_tool:AddedToSelection( entity )
...
function switch_all_map_picker_tool:RemovedFromSelection( entity )
    EntityService:RemoveMaterial(entity, "selected" )
end

function switch_all_map_picker_tool:FilterSelectedEntities( selectedEntities )
...
function switch_all_map_picker_tool:OnUpdate()
end

function switch_all_map_picker_tool:OnActivateSelectorRequest()
...
function switch_all_map_picker_tool:ChangeSelector(blueprintName)
...
function switch_all_map_picker_tool:FillLastBuildingsList(defaultModesArray, modeBuildingLastSelected, selector)
...
function switch_all_map_picker_tool:AddBlueprintToLastList(blueprintName, selector)

    LastSelectedBlueprintsListUtils:AddBlueprintToList(self.list_name, selector, blueprintName)
end

function switch_all_map_picker_tool:OnRotateSelectorRequest(evt)
...
function switch_all_map_picker_tool:CheckModeValueExists( selectedMode )
...
function switch_all_map_picker_tool:OnRelease()
...
return switch_all_map_picker_tool
