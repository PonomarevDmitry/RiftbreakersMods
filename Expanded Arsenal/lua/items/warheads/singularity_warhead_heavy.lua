...
class 'singularity_warhead_heavy' ( item )

function singularity_warhead_heavy:__init()
	item.__init(self,self)
end

function singularity_warhead_heavy:OnInit()
	...
	self.closedTime = 0.22
	self.openingTime = 0.3
	self.openedTime = 3.0
	self.closingTime = 0.2
	
	-- Damage parameters
	self.initialBlast =  self.data:GetStringOrDefault("initial_blast","items/warheads/singularity_warhead_heavy_initial_blast")
	self.damage =  self.data:GetStringOrDefault("damage","items/warheads/singularity_warhead_heavy_damage")
	self.idleSlowDown = self.data:GetStringOrDefault("slowdown","items/warheads/singularity_warhead_heavy_slowdown")
	
	-- Graphics parameters
	self.idleRadius = self.data:GetStringOrDefault("radius","items/warheads/singularity_warhead_heavy_radius")
	self.closedEffect = self.data:GetStringOrDefault("closed_effect","items/warheads/singularity_warhead_heavy_closed")
	self.refract = self.data:GetStringOrDefault("refract_effect","items/warheads/singularity_warhead_heavy_refract")
	
	-- Sound parameters
	self.initSound = self.data:GetStringOrDefault("init_sound","items/warheads/singularity_warhead_heavy_init_sound")
	self.openSound = self.data:GetStringOrDefault("open_sound","items/warheads/singularity_warhead_heavy_open_sound")
	self.idleSound = self.data:GetStringOrDefault("idle_sound","items/warheads/singularity_warhead_heavy_idle_sound")
	self.closeSound = self.data:GetStringOrDefault("close_sound","items/warheads/singularity_warhead_heavy_close_sound")
end

function singularity_warhead_heavy:OnClosedEnter( state )
...
function singularity_warhead_heavy:OnClosedExecute( state )
...
function singularity_warhead_heavy:OnClosedExit( state )
	self.fsm:ChangeState( "opening" )
	EntityService:RemoveEntity( self.closedEnt )
end

function singularity_warhead_heavy:SetItemCreator(entity)
...
function singularity_warhead_heavy:OnOpeningEnter( state )
...
function singularity_warhead_heavy:OnOpeningExecute( state )
...
function singularity_warhead_heavy:OnOpeningExit( state )
	self.fsm:ChangeState( "opened" )
end

function singularity_warhead_heavy:OnOpenedEnter( state )
...
function singularity_warhead_heavy:OnOpenedExecute( state )
...
function singularity_warhead_heavy:OnOpenedExit( state )
...
function singularity_warhead_heavy:OnClosingEnter( state )
	state:SetDurationLimit( self.closingTime )
end

function singularity_warhead_heavy:OnClosingExecute( state )
	local currentProgress = ( state:GetDuration() / self.closingTime )
	local scale = 1.30 - ( 1.3 * ( 1 - ( ( math.cos( 3.14 * currentProgress ) + 1 ) / 2 ) ) )
	EntityService:SetScale( self.entity, scale, scale, scale )
end

function singularity_warhead_heavy:OnClosingExit( state )
	EntityService:RemoveEntity( self.entity )
end

return singularity_warhead_heavy
