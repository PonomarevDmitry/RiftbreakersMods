...
class 'singularity_warhead' ( item )

function singularity_warhead:__init()
	item.__init(self,self)
end

function singularity_warhead:OnInit()
	...
	-- Damage parameters
	self.initialBlast =  self.data:GetStringOrDefault("initial_blast","items/warheads/singularity_warhead_initial_blast")
	self.damage =  self.data:GetStringOrDefault("damage","items/warheads/singularity_warhead_damage")
	self.idleSlowDown = self.data:GetStringOrDefault("slowdown","items/warheads/singularity_warhead_slowdown")
...
function singularity_warhead:OnClosedEnter( state )
...
function singularity_warhead:OnClosedExecute( state )
...
function singularity_warhead:OnClosedExit( state )
	self.fsm:ChangeState( "opening" )
	EntityService:RemoveEntity( self.closedEnt )
end

function singularity_warhead:SetItemCreator(entity)
...
function singularity_warhead:OnOpeningEnter( state )
...
function singularity_warhead:OnOpeningExecute( state )
...
function singularity_warhead:OnOpeningExit( state )
	self.fsm:ChangeState( "opened" )
end

function singularity_warhead:OnOpenedEnter( state )
...
function singularity_warhead:OnOpenedExecute( state )
...
function singularity_warhead:OnOpenedExit( state )
...
function singularity_warhead:OnClosingEnter( state )
	state:SetDurationLimit( self.closingTime )
end

function singularity_warhead:OnClosingExecute( state )
	local currentProgress = ( state:GetDuration() / self.closingTime )
	local scale = 1.30 - ( 1.3 * ( 1 - ( ( math.cos( 3.14 * currentProgress ) + 1 ) / 2 ) ) )
	EntityService:SetScale( self.entity, scale, scale, scale )
end

function singularity_warhead:OnClosingExit( state )
	EntityService:RemoveEntity( self.entity )
end

return singularity_warhead
