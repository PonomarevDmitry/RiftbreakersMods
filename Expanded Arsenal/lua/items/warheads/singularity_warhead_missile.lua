...
class 'singularity_warhead_missile' ( item )

function singularity_warhead_missile:__init()
	item.__init(self,self)
end

function singularity_warhead_missile:OnInit()
	...
	self.closedTime = 0.2
	self.openingTime = 0.25
	self.openedTime = 3.0
	self.closingTime = 0.22
	
	-- Damage parameters
	self.initialBlast =  self.data:GetStringOrDefault("initial_blast","items/warheads/singularity_warhead_missile_initial_blast")
	self.damage =  self.data:GetStringOrDefault("damage","items/warheads/singularity_warhead_missile_damage")
	self.idleSlowDown = self.data:GetStringOrDefault("slowdown","items/warheads/singularity_warhead_missile_slowdown")
	
	-- Graphics parameters
	self.idleRadius = self.data:GetStringOrDefault("radius","items/warheads/singularity_warhead_missile_radius")
	self.closedEffect = self.data:GetStringOrDefault("closed_effect","items/warheads/singularity_warhead_missile_closed")
	self.refract = self.data:GetStringOrDefault("refract_effect","items/warheads/singularity_warhead_missile_refract")
...
function singularity_warhead_missile:OnClosedEnter( state )
...
function singularity_warhead_missile:OnClosedExecute( state )
...
function singularity_warhead_missile:OnClosedExit( state )
	self.fsm:ChangeState( "opening" )
	EntityService:RemoveEntity( self.closedEnt )
end

function singularity_warhead_missile:SetItemCreator(entity)
...
function singularity_warhead_missile:OnOpeningEnter( state )
...
function singularity_warhead_missile:OnOpeningExecute( state )
...
function singularity_warhead_missile:OnOpeningExit( state )
	self.fsm:ChangeState( "opened" )
end

function singularity_warhead_missile:OnOpenedEnter( state )
...
function singularity_warhead_missile:OnOpenedExecute( state )
...
function singularity_warhead_missile:OnOpenedExit( state )
...
function singularity_warhead_missile:OnClosingEnter( state )
	state:SetDurationLimit( self.closingTime )
end

function singularity_warhead_missile:OnClosingExecute( state )
	local currentProgress = ( state:GetDuration() / self.closingTime )
	local scale = 1.30 - ( 1.3 * ( 1 - ( ( math.cos( 3.14 * currentProgress ) + 1 ) / 2 ) ) )
	EntityService:SetScale( self.entity, scale, scale, scale )
end

function singularity_warhead_missile:OnClosingExit( state )
	EntityService:RemoveEntity( self.entity )
end

return singularity_warhead_missile
