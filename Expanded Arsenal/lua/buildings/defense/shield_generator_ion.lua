...
class 'shield_generator_ion' ( building )

function shield_generator_ion:__init()
	building.__init(self,self)
end

function shield_generator_ion:OnInit()
	self.active = true
	self.selected = {}
	-- Defaults:
	self.interval = 1
	...
	self.shieldBp = self.data:GetStringOrDefault("shield_bp", "buildings/defense/shield_generator_ion/shield_ion")
...
function shield_generator_ion:OnWorkInProgress( state )
...
function shield_generator_ion:OnWorkExit( state )
	for i = #self.selected,1,-1  do		
		ItemService:RemoveHealthLink( self.selected[i], self.healthChild )
	end		
	Clear( self.selected )
...
function shield_generator_ion:OnBuildingEnd()
end

function shield_generator_ion:OnActivate()
	self.fsm:ChangeState("working")
end

function shield_generator_ion:OnDeactivate()
...
function shield_generator_ion:OnLoad()
	...
	if ( self.shieldBp == nil ) then
		if ( self.data:HasString("shield_bp")) then
			...
			if ( lvl == 1 ) then
				self.shieldBp = "buildings/defense/shield_generator_ion/shield_ion"
			else
				self.shieldBp = "buildings/defense/shield_generator_ion/shield_ion" .. "_lvl_" .. tostring(lvl)
			end
		end
	end
end

return shield_generator_ion
