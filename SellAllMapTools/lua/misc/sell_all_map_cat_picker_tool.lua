...
class 'sell_all_map_cat_picker_tool' ( tool )

function sell_all_map_cat_picker_tool:__init()
    tool.__init(self,self)
end

function sell_all_map_cat_picker_tool:OnPreInit()
    self.initialScale = { x=1, y=1, z=1 }
end

function sell_all_map_cat_picker_tool:GetScaleFromDatabase()
    return { x=1, y=1, z=1 }
end

function sell_all_map_cat_picker_tool:OnInit()
...
function sell_all_map_cat_picker_tool:SpawnCornerBlueprint()
...
function sell_all_map_cat_picker_tool:UpdateMarker()
    ...
    elseif ( self.selectedCategory ~= nil and self.selectedCategory ~= "" ) then
        ...
        local messageText = "${gui/hud/sell_all_map/current_building}"
...
function sell_all_map_cat_picker_tool:AddedToSelection( entity )
...
function sell_all_map_cat_picker_tool:RemovedFromSelection( entity )
...
function sell_all_map_cat_picker_tool:FilterSelectedEntities( selectedEntities )
...
function sell_all_map_cat_picker_tool:OnUpdate()
end

function sell_all_map_cat_picker_tool:OnActivateSelectorRequest()
...
function sell_all_map_cat_picker_tool:ChangeSelector(category)
...
function sell_all_map_cat_picker_tool:FillLastCategoriesList(defaultModesArray, modeSelectLast, selector)
...
function sell_all_map_cat_picker_tool:AddCategoryToLastList(category, selector)

    LastSelectedBlueprintsListUtils:AddStringToList(self.list_name, selector, category)
end

function sell_all_map_cat_picker_tool:OnRotateSelectorRequest(evt)
...
function sell_all_map_cat_picker_tool:CheckModeValueExists( selectedMode )
...
return sell_all_map_cat_picker_tool
