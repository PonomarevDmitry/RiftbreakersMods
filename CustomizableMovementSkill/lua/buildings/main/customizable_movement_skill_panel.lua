...
class 'customizable_movement_skill_panel' ( building )

function customizable_movement_skill_panel:__init()
    building.__init(self,self)
end

function customizable_movement_skill_panel:OnInit()
...
function customizable_movement_skill_panel:OnLoad()
...
function customizable_movement_skill_panel:RegisterEventHandlers()
...
function customizable_movement_skill_panel:OperatePlayerItemConfiguration( player_id )
    ...
    local movementSkillItem = ItemService:GetFirstItemForBlueprint( player, self.skillName )
    if ( movementSkillItem == INVALID_ID ) then
        movementSkillItem = ItemService:AddItemToInventory( player, self.skillName )
    end
    ...
    if ( equipmentComponent ~= nil ) then
        ...
        for i=1,slots.count do
            ...
            local keyName = "customizable_movement_skill_MOD_" .. tostring(i + modDelta)
...
function customizable_movement_skill_panel:OnItemEquippedEvent( evt )
    ...
    if ( movementSkillItem ~= INVALID_ID ) then
        ...
        if ( database ~= nil ) then
            ...
            local keyName = "customizable_movement_skill_MOD_" .. tostring(slotNumber + modDelta)

            database:SetString(keyName, itemBlueprintName)

            EntityService:CreateComponent( movementSkillItem, "NetReplicateNextFrameComponent")
        end
    end
...
function customizable_movement_skill_panel:OnItemUnequippedEvent( evt )
...
function customizable_movement_skill_panel:GetSlotNumber( slotName )
...
return customizable_movement_skill_panel
