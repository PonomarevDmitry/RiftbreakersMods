...
RegisterGlobalEventHandler("OperateActionMapperRequest", function(evt)
    ...
    local stringNumber = string.find( mapperName, "CustomizableMovementSkillPanelOnOperateActionMenu" )
    ...
    if ( stringNumber == 1 ) then
        ...
        local skillName = blueprintDatabase:GetStringOrDefault("skill_name", "items/skills/customizable_movement_skill_1_item")

        local movementSkillItem = ItemService:GetFirstItemForBlueprint( player, skillName )
        if ( movementSkillItem == INVALID_ID ) then
            movementSkillItem = ItemService:AddItemToInventory( player, skillName )
        end
